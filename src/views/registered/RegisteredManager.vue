  ...
  <div class="table">
    ...
    <el-table :data="list" v-loading="listLoading" element-loading-text="正在加载数据..." borde fit highlight-current-row style="width:100%">
      ...
      <el-table-column align="center" label="操作" width="180px">
        <template slot-scope="scope">
          <el-button v-if="scope.row.registeredStateCode==0" type="warning" icon="circle-close" size="small" @click="handleCancel(scope.row)">取消</el-button>
          <el-button v-if="scope.row.registeredStateCode==0" type="danger" icon="circle-close" size="small" @click="handleBreakPromise(scope.row)">爽约</el-button>
        </template>
      </el-table-column>
    </el-table>
...
<script>
import { getRegistereds, cancelRegistered, breakPromiseRegistered } from '@/api/registered'
...
export default {
  ...
  methods: {
    ...
    cancelRegistered (id) {
      cancelRegistered(id).then(response => {
    ...
    breakPromiseRegistered (id) {
      breakPromiseRegistered(id).then(response => {
        if (response.status === 204) {
          this.$notify({
            title: '成功',
            message: '设置爽约成功',
    ...
    handleCancel (row) {
      this.$confirm('确定要取消当前预约？', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        this.cancelRegistered(row.id)
      }).catch(() => {
        this.$message({
          type: 'info',
          message: '已取消操作!'
    ...
    handleBreakPromise (row) {
      this.$confirm('确定当前预约已经爽约？', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        this.breakPromiseRegistered(row.id)
      }).catch(() => {
        this.$message({
          type: 'info',
          message: '已取消操作!'
