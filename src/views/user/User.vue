<template>
  <div class="table">
    ...
    <div class="handle-box">
      所属部门:
      <el-cascader :options="orgSelects" :props="defaultProps" v-model="orgSelected"  @change="handleChange" change-on-select>
      </el-cascader>
...
export default {
  ...
  methods: {
    ...
    getRegisteredRankSelects () {
      getDictonarySelect('RegisteredRank').then(response => {
        this.registeredRankSelects = response.data
      })
    },
    getTemp (id) {
      getUser(id).then(response => {
        this.temp = {
          ...
          registeredRankCode: response.data.registeredRankCode,
          registeredRankName: response.data.registeredRankName,
    ...
    createTemp () {
      this.$refs.temp.validate(valid => {
        if (valid) {
          ...
          this.temp.registeredRankName = this.registeredRankSelects.find(item => item.code === this.temp.registeredRankCode).name
